...
水滴待办是一款本地优先的 macOS 时间感知待办应用。它把临近截止的任务显示为 MacBook 刘海下方的液态水滴，用颜色和位置传达紧迫程度，并通过鱼缸、废墟与召回交互呈现任务的完整生命周期。

当前版本：`0.1.0 (1)`，MVP Alpha。
...
- AppKit：刘海、命中区域、悬停卡片、鱼缸及跨窗口动画 Panel
...
项目当前处于 MVP Alpha 内测阶段。建议提交问题时附上版本与构建号、Mac 机型、macOS 版本、显示器与全屏状态、复现步骤以及期望/实际结果；请勿在 Issue 中公开完整任务 JSON。
